<?php
/**
 * @version $Id: methodhelpdesk.class.php 455 2018-06-04 16:27:26Z yllen $
 -------------------------------------------------------------------------
 LICENSE
   ...
   **/
   static function methodCreateTicket($params=[], $protocol) {
      ...
      if (isset($params['location'])) {
         foreach ($DB->request("glpi_locations", [$key => $data['entities_id']]) as $loc) {
            $locs[$loc['id']] = $loc['id'];
         }
   ...
   static function methodListTickets($params, $protocol) {
      ...
      if (isset($params['count'])) {
         ...
         foreach ($DB->request($query) as $data) {
            ...
            if (isset($params['id2name'])) {
               if ($data['itemtype'] && ($item = $dbu->getItemForItemtype($data['itemtype']))) {
...
//New methods
   ...
   static function methodsetTicketAssign($params, $protocol) {
      ...
      if (isset($params['group'])) {
         $query = ['FROM'  => 'glpi_groups',
                   'WHERE' => [$dbu->getEntitiesRestrictCriteria("glpi_groups", '',
                                                                 $data['entities_id'], true),
                               'is_assign'   => 1]];
   ...
   static function methodAddTicketTask($params, $protocol) {
      ...
      if (isset($params['tech']) && !is_numeric($param['tech'])
      ...
      if (isset($params['group']) && !is_numeric($param['group'])
