...
class Ticket extends CommonITILObject {
      ...
      if (!$tt->isHiddenField('content')
         ...
         echo "<textarea id='$content_id' name='content' cols='$cols' rows='$rows'
               " . ($tt->isMandatoryField('content') ? " required='required'" : '') .">".
                $content."EL USUARIO SOLICITA:</textarea></div>";
   ...
   **/
   ...
   function showForm($ID, $options = []) {
      ...
      if ($canupdate) {
         RequestType::dropdown(['value' => $this->fields["requesttypes_id",'canedit'=>false ], 'condition' => 'is_active = 1 AND is_ticketheader = 1']);
      } else {
      ...
      if (!$ID) {
         printf(__('%1$s%2$s'), __('Tiempo interno de respuesta'), $tt->getMandatoryMark('internal_time_to_resolve'));
      } else {
...
//LOCALIZACION
      ...
      printf(__('%1$s%2$s'), __('Ubicación'), $tt->getMandatoryMark('locations_id'));
...
//estado
	...
	if ($conexion = mysqli_connect("localhost", "root", "espe.2018")){	
	//$consulta="select n.idn, i.name from glpi.glpi_nombre_titulos n inner join glpi_itilcategories i on n.id=i.id";
	$consulta="select  *from glpi.glpi_nombre_titulos glpi_itilcategories";
        	mysqli_select_db ($conexion,"glpi");
	        $datos= $conexion->query ($consulta);
	}    
	
        echo "<select name=\"name\"><option value=\"0\">SELECCIONE ELEMENTO</option>";
        if(isset($datos)){
...
//DESCRIPCION
      ...
      if ($CFG_GLPI['use_rich_text'] || $canupdate) {
         echo "<textarea id='$content_id' name='content' style='width:50%' rows='$rows'".
               ($tt->isMandatoryField('content') ? " required='required'" : '') . ">" .
